...
interface Escuela { id: number; nombre: string; departamento_id: number; departamento: Departamento }
interface FormState { nombre: string; departamento_id: string }
const EMPTY: FormState = { nombre: "", departamento_id: "" };
...
export default function EscuelasPage() {
  ...
  async function load() {
    try {
      setLoading(true);
      setError(null);
      const [escuelas, deps] = await Promise.all([
  ...
  function openEdit(item: Escuela) {
    setEditing(item);
    setForm({ nombre: item.nombre, departamento_id: item.departamento_id.toString() });
    setFormError(null);
    setOpen(true);
  }
  ...
  async function handleSave() {
    ...
    if (!form.departamento_id) { setFormError("Selecciona un departamento"); return; }
    try {
      setSaving(true);
      setFormError(null);
      const body = { nombre: form.nombre, departamento_id: parseInt(form.departamento_id) };
      if (editing) {
        await apiFetch(`/api/escuelas/${editing.id}`, { method: "PUT", body: JSON.stringify(body) });
      } else {
        await apiFetch("/api/escuelas", { method: "POST", body: JSON.stringify(body) });
      }
      setOpen(false);
      await load();
    } catch (e) {
  ...
  async function handleDelete() {
    ...
    try {
      setDeleting(true);
      await apiFetch(`/api/escuelas/${deleteTarget.id}`, { method: "DELETE" });
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-8 py-5 border-b border-border shrink-0">
        <div>
          <h1 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Escuelas profesionales</h1>
          <p className="text-sm text-zinc-500 mt-0.5">Escuelas y carreras académicas</p>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={4} className="px-8 py-12 text-sm text-zinc-400 text-center">Cargando...</td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan={4} className="px-8 py-12 text-sm text-zinc-400 text-center">No hay escuelas registradas</td></tr>
            ) : items.map((item) => (
              <tr key={item.id} className="border-b border-black/[0.04] dark:border-white/5 hover:bg-zinc-50 dark:hover:bg-white/[0.02] transition-colors duration-150">
                ...
                <td className="px-8 py-3.5 text-sm text-zinc-500 dark:text-zinc-400">{item.departamento.nombre}</td>
                <td className="px-8 py-3.5 text-sm text-zinc-400 dark:text-zinc-500">{item.departamento.facultad.nombre}</td>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-zinc-900 border border-border rounded-[10px] shadow-xl w-full max-w-md mx-4 p-6">
            <h2 className="text-base font-semibold text-zinc-900 dark:text-zinc-100 mb-5">{editing ? "Editar escuela" : "Nueva escuela"}</h2>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-400">Nombre</label>
                <input value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })} placeholder="Ej. Ingeniería en Sistemas" className={inputCls} />
              </div>
              <div className="space-y-1.5">
                ...
                <Select value={form.departamento_id} onValueChange={(v) => setForm({ ...form, departamento_id: v ?? "" })}>
                  <SelectTrigger className="h-9 text-sm"><SelectValue placeholder="Seleccionar departamento">{form.departamento_id ? departamentos.find((d) => d.id.toString() === form.departamento_id)?.nombre : undefined}</SelectValue></SelectTrigger>
                  <SelectContent>{departamentos.map((d) => <SelectItem key={d.id} value={d.id.toString()}>{d.nombre} — {d.facultad.nombre}</SelectItem>)}</SelectContent>
                </Select>
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-zinc-900 border border-border rounded-[10px] shadow-xl w-full max-w-sm mx-4 p-6">
            <h2 className="text-base font-semibold text-zinc-900 dark:text-zinc-100 mb-2">Eliminar escuela</h2>
