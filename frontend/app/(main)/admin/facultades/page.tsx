...
export default function FacultadesPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<Facultad | null>(null);
  ...
  async function load() {
    try {
      setLoading(true);
      setError(null);
      const data = await apiFetch<Facultad[]>("/api/facultades");
      setItems(data);
    } catch (e) {
  ...
  function openEdit(item: Facultad) { setEditing(item); setForm({ nombre: item.nombre }); setFormError(null); setOpen(true); }
  ...
  async function handleSave() {
    ...
    try {
      ...
      setFormError(null);
      if (editing) {
        await apiFetch(`/api/facultades/${editing.id}`, { method: "PUT", body: JSON.stringify(form) });
      } else {
        await apiFetch("/api/facultades", { method: "POST", body: JSON.stringify(form) });
      }
      setOpen(false);
      await load();
    } catch (e) {
  ...
  async function handleDelete() {
    ...
    try {
      setDeleting(true);
      await apiFetch(`/api/facultades/${deleteTarget.id}`, { method: "DELETE" });
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-8 py-5 border-b border-border shrink-0">
        <div>
          <h1 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Facultades</h1>
          <p className="text-sm text-zinc-500 mt-0.5">Gestión de facultades</p>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={2} className="px-8 py-12 text-sm text-zinc-400 text-center">Cargando...</td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan={2} className="px-8 py-12 text-sm text-zinc-400 text-center">No hay facultades registradas</td></tr>
            ) : items.map((item) => (
              <tr key={item.id} className="border-b border-black/[0.04] dark:border-white/5 hover:bg-zinc-50 dark:hover:bg-white/[0.02] transition-colors duration-150">
                <td className="px-8 py-3.5 text-sm text-zinc-800 dark:text-zinc-200">{item.nombre}</td>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-zinc-900 border border-border rounded-[10px] shadow-xl w-full max-w-md mx-4 p-6">
            <h2 className="text-base font-semibold text-zinc-900 dark:text-zinc-100 mb-5">{editing ? "Editar facultad" : "Nueva facultad"}</h2>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-400">Nombre</label>
                <input value={form.nombre} onChange={(e) => setForm({ nombre: e.target.value })} placeholder="Ej. Facultad de Ciencias Exactas" className={inputCls} />
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-zinc-900 border border-border rounded-[10px] shadow-xl w-full max-w-sm mx-4 p-6">
            <h2 className="text-base font-semibold text-zinc-900 dark:text-zinc-100 mb-2">Eliminar facultad</h2>
